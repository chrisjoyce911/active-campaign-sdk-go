...
func TestService_DeleteAccount(t *testing.T) {
	tests := []struct {
		...
		{name: "ok", mockResp: &client.APIResponse{StatusCode: 200}, mockBody: []byte(`{"success":true}`), id: "a1", wantStatus: 200},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			apiResp, err := svc.DeleteAccount(context.Background(), tt.id)
